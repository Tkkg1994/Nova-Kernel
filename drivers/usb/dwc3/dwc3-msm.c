...
static void dwc3_restart_usb_work(struct work_struct *w)
{
	...
	queue_delayed_work(system_power_efficient_wq, &mdwc->resume_work, 0);
...
void msm_dwc3_restart_usb_session(struct usb_gadget *gadget)
{
	...
	dev_dbg(mdwc->dev, "%s\n", __func__);
	queue_work(system_power_efficient_wq, &mdwc->restart_usb_work);
}
...
static void dwc3_msm_notify_event(struct dwc3 *dwc, unsigned event)
{
	...
	case DWC3_CONTROLLER_ERROR_EVENT:
		 ...
		 * error event.
		 */
		queue_work(system_power_efficient_wq, &mdwc->usb_block_reset_work);
		break;
...
static void dwc3_chg_detect_work(struct work_struct *w)
{
	...
	queue_delayed_work(system_power_efficient_wq, &mdwc->chg_work, delay);
}
...
static void dwc3_start_chg_det(struct dwc3_charger *charger, bool start)
{
	...
	mdwc->chg_state = USB_CHG_STATE_UNDEFINED;
	charger->chg_type = DWC3_INVALID_CHARGER;
	queue_delayed_work(system_power_efficient_wq, &mdwc->chg_work, 0);
}
...
static irqreturn_t dwc3_pmic_id_irq(int irq, void *data)
{
	...
	if (mdwc->id_state != id) {
		mdwc->id_state = id;
		queue_work(system_power_efficient_wq, &mdwc->id_work);
	}
...
static void dwc3_init_adc_work(struct work_struct *w)
{
	...
	if (IS_ERR(mdwc->adc_tm_dev)) {
		if (PTR_ERR(mdwc->adc_tm_dev) == -EPROBE_DEFER)
			queue_delayed_work(system_power_efficient_wq, to_delayed_work(w),
...
static int dwc3_msm_probe(struct platform_device *pdev)
{
	...
	if (mdwc->ext_xceiv.otg_capability) {
		...
		if (mdwc->pmic_id_irq <= 0) {
			...
			queue_work(system_power_efficient_wq, &mdwc->init_adc_work.work);
